...
class Donor():
    ...
    def get_donor_details(session_email):
        ...
        keys = ["user_id", "first_name", "middle_name", "last_name", "email", "password",
         "phone_number", "org_name", "about", "street", "barangay" , "city", "image_link",
          "user_role", "public_profile", "show_donor_name", "notifications"]
    ...
    def get_my_donations(self, user_id, role='donor'):
        ...
        for item in result:
            ...
            if donation_id not in donations:
                ...
                donations[donation_id]['items'].append([category, item[9], quantity, image])
        return donations
    ...
    def get_collectors(session_email) -> list:
        ...
        for collector in result:
            ...
            if org_name and org_name not in collectors:
                ...
                collectors[collector_name] = [collector_image, collector_id, collector[6]]
            elif not org_name:
    ...
    def get_donors_of_the_week():
        ...
        cursor.execute(query)
        result = cursor.fetchall()
        donors = [list(donor) for donor in result]
        return donors

    @staticmethod
    def invitations(user_id):
        query = f'''
            select invites.invite_id, invites.user_id, invites.collector_id, users.first_name, users.last_name, org_name
            from invites join users
            on invites.collector_id=users.user_id and invites.user_id={user_id}
        '''
        cursor.execute(query)
    ...
    @staticmethod
    def accept_invite(invite_id, user_id, collector_id):
        # add as member
        # then delete invite from 'invites' table
        query1 = f'''
            INSERT INTO 
            members(user_id, collector_id) 
            VALUES({user_id}, {collector_id})
        '''
        cursor.execute(query1)
        db.commit()

        query2 = f"DELETE FROM invites WHERE invite_id={invite_id}"
        cursor.execute(query2)
        db.commit()
    ...
    def decline_invite(invite_id):
        query = f"DELETE FROM invites WHERE invite_id={invite_id}"
