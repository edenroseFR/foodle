...
def register() -> str:
    ...
    if form.validate_on_submit():
        ...
        flash(
            "Registration failed. E-mail, Phone Number, and Organization Name must be unique.",
            "info",
        )
...
def reset_password() -> str:
    ...
    if request.method == "POST":
        ...
        if registered_email:
            ...
            return render_template(
                "/login/get_email.html", form=form, reg_form=reg_form
            )
...
def verify_code():
    ...
    if request.method == "POST":
        ...
        if user_input == session["reset_code"]:
            return render_template(
                "/login/new_password.html", form=form, reg_form=reg_form
            )
        else:
